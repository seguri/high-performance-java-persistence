...
public final class CryptoUtilsTest {
  ...
  private final BoundedConcurrentHashMap<Long, String> encryptCache =
      new BoundedConcurrentHashMap<>(100_000, 20, BoundedConcurrentHashMap.Eviction.LIRS);

  private final BoundedConcurrentHashMap<String, Long> decryptCache =
      new BoundedConcurrentHashMap<>(100_000, 20, BoundedConcurrentHashMap.Eviction.LIRS);
  ...
  public void testPerformance() {
    ...
    LongStream.rangeClosed(1, MAX_COUNT)
        .forEach(
            i -> {
  ...
  private void warmUp() {
    LongStream.rangeClosed(1, MAX_COUNT / 10)
        .forEach(
            i -> {
