...
public interface ExceptionUtil {
  ...
  List<Class<? extends Exception>> LOCK_TIMEOUT_EXCEPTIONS =
      Arrays.asList(
  ...
  static boolean isMVCCAnomalyDetection(Throwable e) {
    ...
    do {
      ...
      if (cause
              .getMessage()
              .contains("ORA-08177: can't serialize access for this transaction") // Oracle
          || lowerCaseMessage.contains(
              "could not serialize access due to concurrent update") // PSQLException
          || lowerCaseMessage.contains(
              "ould not serialize access due to read/write dependencies among transactions") // PSQLException
          || lowerCaseMessage.contains(
              "snapshot isolation transaction aborted due to update conflict") // SQLServerException
          || lowerCaseMessage.contains("kconflict") // YugabyteDB
          || lowerCaseMessage.contains(
              "unknown transaction, could be recently aborted") // YugabyteDB
