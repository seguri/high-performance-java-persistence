...
public class SlowQueryLogTest extends AbstractTest {
  ...
  public void afterInit() {
    doInJPA(
        entityManager -> {
          LongStream.rangeClosed(1, 50 * 100)
              .forEach(
                  i -> {
                    entityManager.persist(
                        new Post()
                            ...
                            .setTitle(
                                String.format(
                                    "High-Performance Java Persistence book - page %d review", i))
    ...
    doInJPA(
        entityManager -> {
          ...
          postQuery
              .where(
                  builder.like(
                      builder.lower(post.get("title")), "%Java%book%review%".toLowerCase()))
              .orderBy(builder.desc(post.get("createdOn")));
