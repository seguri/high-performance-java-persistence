...
public class PivotTest extends AbstractOracleIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Service.class, Component.class, Property.class};
  }
  ...
  public void test() {
    ...
    doInJPA(
        entityManager -> {
          ...
          databaseName.setId(new PropertyId(apollo, dataSource, "databaseName"));
          ...
          serverName.setId(new PropertyId(apollo, dataSource, "serverName"));
          serverName.setValue("192.168.0.5");
          entityManager.persist(serverName);

          Property user = new Property();
          user.setId(new PropertyId(apollo, dataSource, "username"));
          user.setValue("postgres");
          entityManager.persist(user);

          Property password = new Property();
          password.setId(new PropertyId(apollo, dataSource, "password"));
          password.setValue("admin");
          entityManager.persist(password);

          Component flexyPoolDataSource =
              entityManager.find(Component.class, "flexyPoolDataSource");
          ...
          jmxAutoStart.setId(
              new PropertyId(
                  apollo, flexyPoolDataSource, "flexy.pool.metrics.reporter.jmx.auto.start"));
    ...
    doInJPA(
        entityManager -> {
          ...
          Property databaseName = new Property();
          databaseName.setId(new PropertyId(artemis, dataSource, "databaseName"));
          databaseName.setValue("high_performance_java_persistence");
          entityManager.persist(databaseName);

          Property serverName = new Property();
          serverName.setId(new PropertyId(artemis, dataSource, "url"));
          serverName.setValue("jdbc:oracle:thin:@192.169.0.6:1521/hpjp");
          entityManager.persist(serverName);

          Property user = new Property();
          user.setId(new PropertyId(artemis, dataSource, "username"));
          user.setValue("oracle");
          entityManager.persist(user);

          Property password = new Property();
          password.setId(new PropertyId(artemis, dataSource, "password"));
          password.setValue("admin");
          entityManager.persist(password);
          ...
          reporterLogMillis.setId(
              new PropertyId(
                  artemis, flexyPoolDataSource, "flexy.pool.metrics.reporter.log.millis"));
    ...
    doInJPA(
        entityManager -> {
          List<Property> componentProperties =
              entityManager
                  .createQuery(
                      "SELECT p " + "FROM Property p " + "WHERE p.id.component.name = :name",
                      Property.class)
    ...
    doInJPA(
        entityManager -> {
          List<Object[]> componentProperties =
              entityManager
                  .createNativeQuery(
                      "SELECT "
