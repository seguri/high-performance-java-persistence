...
public class SQLServerDeadLockTest extends AbstractTest {
  ...
  public void destroy() {
    executeStatement(
        "ALTER DATABASE [high_performance_java_persistence] SET READ_COMMITTED_SNAPSHOT OFF");
    super.destroy();
  }
   ...
   *
   * <p>DBCC TRACEON (1204, 1222, -1)
   *
   * <p>And, read the error log using the following SP:
   *
   * <p>sp_readerrorlog
   */
  @Test
  public void testDeadLock_1204() {
    ...
    try {
      ...
      LOGGER.info(
          ...
          afterDeadLockErrorLogLines.stream()
              .map(ErrorLogMessage::getMessage)
      ...
      return session
          ...
          .setResultTransformer(
              (ListResultTransformer)
                  (tuple, aliases) -> new ErrorLogMessage((Date) tuple[0], (String) tuple[2]))
          .getResultList();
    }
  }
