...
public class PlanCacheSizePerformanceTest extends AbstractTest {
  ...
  protected void compileQueries(
      Function<EntityManager, Query> query1, Function<EntityManager, Query> query2) {
  ...
  protected Query getEntityQuery2(EntityManager entityManager) {
    return entityManager.createQuery(
        "select c " + "from PostComment c " + "join fetch c.post p " + "where p.title like :title");
  }
  ...
  @NamedNativeQuery(
      ...
      query = "select c.review " + "from post_comment c " + "where c.id > :id ")
  @NamedNativeQuery(
