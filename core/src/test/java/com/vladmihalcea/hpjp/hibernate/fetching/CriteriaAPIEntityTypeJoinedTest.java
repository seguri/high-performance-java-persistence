...
public class CriteriaAPIEntityTypeJoinedTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class[] {Topic.class, Post.class, Announcement.class};
  }
  ...
  public void test() {
    doInJPA(
        entityManager -> {
          ...
          criteria.where(builder.equal(root.get("owner"), "Vlad"));

          List<Topic> topics = entityManager.createQuery(criteria).getResultList();
    ...
    doInJPA(
        entityManager -> {
          ...
          criteria.where(
              builder.and(
                  builder.equal(root.get("owner"), "Vlad"), builder.equal(root.type(), sublcass)));
