...
public class BlazePersistenceCriteriaTest extends AbstractTest {
        ...
        entityManager -> {
          Post post =
              new Post()
                  ...
                  .addComment(
                      new PostComment().setId(1L).setReview("Best book on JPA and Hibernate!"))
                  .addComment(new PostComment().setId(2L).setReview("A great reference book."))
                  .addComment(
                      new PostComment()
                          .setId(3L)
          ...
          entityManager.persist(new PostDetails().setPost(post).setCreatedBy("Vlad Mihalcea"));
        ...
        entityManager -> {
          List<Tuple> tuples =
              cbf.create(entityManager, Tuple.class)
                  ...
                  .select("pc1.post.id")
                  .bind("maxId")
                  .select("MAX(pc1.id) OVER (PARTITION BY pc1.post.id)")
                  .end()
                  .joinOn(Post.class, "p", JoinType.INNER)
                  .onExpression("p.id = pc2.postId")
                  .end()
                  .where("pc2.id")
                  .eqExpression("pc2.maxId")
                  .select("p.id", "post_id")
    ...
    doInJPA(
        entityManager -> {
          List<Tuple> tuples =
              cbf.create(entityManager, Tuple.class)
                  .from(Post.class, "p")
                  .leftJoinOn(PostComment.class, "pc")
                  .onExpression("pc.post = p")
                  .end()
                  .joinOn(PostDetails.class, "pd", JoinType.INNER)
                  .onExpression("pd.post = p")
                  .end()
                  .where("pd.createdBy")
                  .eqExpression(":createdBy")
                  .groupBy("p.title")
                  .select("p.title", "post_title")
  ...
  public void testCriteriaAPIAlternative() {
    ...
    doInJPA(
        entityManager -> {
          List<Post> tuples =
              cbf.create(entityManager, Post.class)
                  .from(Post.class, "p")
                  .where(Post_.TITLE)
                  .like()
                  .expression(":titlePattern")
                  .noEscape()
