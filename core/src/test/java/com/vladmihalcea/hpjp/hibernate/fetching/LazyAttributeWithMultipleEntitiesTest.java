...
public class LazyAttributeWithMultipleEntitiesTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Attachment.class, AttachmentSummary.class};
  }
  ...
  public void test() throws URISyntaxException {
    final Path bookFilePath =
        Paths.get(
            Thread.currentThread().getContextClassLoader().getResource("ehcache.xml").toURI());
    final Path videoFilePath =
        Paths.get(
            Thread.currentThread().getContextClassLoader().getResource("spy.properties").toURI());
