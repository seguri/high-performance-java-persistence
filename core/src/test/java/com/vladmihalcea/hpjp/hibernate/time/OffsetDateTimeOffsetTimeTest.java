...
public class OffsetDateTimeOffsetTimeTest extends AbstractMySQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Notification.class};
  }
  ...
  public void test() {
    ...
    doInJPA(
        entityManager -> {
          Notification notification =
              new Notification()
                  .setId(1L)
                  .setCreatedOn(LocalDateTime.of(2020, 5, 1, 12, 30, 0).atOffset(offset))
    ...
    doInJPA(
        entityManager -> {
          Notification notification = entityManager.find(Notification.class, 1L);

          assertEquals(
              LocalDateTime.of(2020, 5, 1, 12, 30, 0).atOffset(offset),
