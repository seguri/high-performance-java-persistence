...
public class IntegerDiscriminatorDescriptionTest extends AbstractMySQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {
      Topic.class, Post.class, Announcement.class, TopicType.class,
    };
  }
          ...
          for (Class entityClass : entities()) {
            if (Topic.class.isAssignableFrom(entityClass)) {
              ...
              DiscriminatorValue discriminatorValue =
                  (DiscriminatorValue) entityClass.getAnnotation(DiscriminatorValue.class);
              ...
              topicType.setDescription(
                  Topic.class.equals(entityClass)
                      ...
                      : String.format(
                          "%s is a subclass of the Topic base class", entityClass.getSimpleName()));
  ...
  public void test() {
    ...
    doInJPA(
        entityManager -> {
          ...
          assertEquals("Best practices", postTuple.get("content"));
          assertEquals(1, ((Number) postTuple.get("discriminator")).intValue());
          assertEquals(Post.class.getSimpleName(), postTuple.get("type_name"));
          assertEquals(
              "Post is a subclass of the Topic base class", postTuple.get("type_description"));
        });
  }
}
