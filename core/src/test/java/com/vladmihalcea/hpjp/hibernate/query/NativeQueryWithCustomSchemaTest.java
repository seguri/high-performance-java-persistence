...
public class NativeQueryWithCustomSchemaTest extends AbstractPostgreSQLIntegrationTest {
  ...
  public void test() {
    doInJPA(
        entityManager -> {
          ...
          finalRelease.setCreatedOn(
              Timestamp.from(LocalDateTime.of(2016, 8, 25, 9, 0, 0).toInstant(ZoneOffset.UTC)));
    ...
    doInJPA(
        entityManager -> {
          List<Event> events =
              entityManager
                  .createQuery(
                      "select e " + "from Event e " + "where e.createdOn > :timestamp", Event.class)
    ...
    doInJPA(
        entityManager -> {
          List<Event> events =
              entityManager.createNamedQuery("past_30_days_events").getResultList();
        });
  }
  ...
  @NamedNativeQuery(
      name = "past_30_days_events",
      query = "select * " + "from {h-schema}event " + "where age(created_on) > '30 days'",
      resultClass = Event.class)
