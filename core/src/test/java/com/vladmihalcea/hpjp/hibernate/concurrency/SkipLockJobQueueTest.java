...
public class SkipLockJobQueueTest extends AbstractPostgreSQLIntegrationTest {
  ...
  public void testLockContention() {
    ...
    doInJPA(
        entityManager -> {
          assertEquals(
              postCount, getAndLockPosts(entityManager, PostStatus.PENDING, postCount).size());
                ...
                () -> {
                  doInJPA(
                      _entityManager -> {
                        assertEquals(
                            postCount,
                            getAndLockPosts(_entityManager, PostStatus.PENDING, postCount).size());
                      });
                });
          } catch (Exception e) {
            assertTrue(
                ExceptionUtil.rootCause(e)
                    .getMessage()
                    .contains("could not obtain lock on row in relation \"post\""));
          }
        });
  }

  public List<Post> getAndLockPosts(EntityManager entityManager, PostStatus status, int postCount) {
    ...
    doInJPA(
        entityManager -> {
          ...
          List<Post> pendingPosts =
              getAndLockPostsWithSkipLocked(entityManager, PostStatus.PENDING, postCount);
                ...
                doInJPA(
                    _entityManager -> {
                      ...
                      List<Post> _pendingPosts =
                          getAndLockPostsWithSkipLocked(
                              _entityManager, PostStatus.PENDING, postCount);
  ...
  public void testAliceLocksAll() {
    ...
    doInJPA(
        entityManager -> {
          List<Post> pendingPosts =
              getAndLockPostsWithSkipLocked(entityManager, PostStatus.PENDING, 10);
          ...
          executeSync(
              () -> {
                doInJPA(
                    _entityManager -> {
                      List<Post> _pendingPosts =
                          getAndLockPostsWithSkipLocked(_entityManager, PostStatus.PENDING, 2);
  ...
  public void testSkipLockedMaxCountLessThanLockCount() {
    ...
    doInJPA(
        entityManager -> {
          List<Post> pendingPosts =
              getAndLockPostsWithSkipLocked(entityManager, PostStatus.PENDING, 11);
          assertEquals(10, pendingPosts.size());
        });
  }

  public List<Post> getAndLockPostsWithSkipLocked(
      EntityManager entityManager, PostStatus status, int postCount) {
    return entityManager
        .createQuery(
            """
            select p
  ...
  private List<Post> getAndLockPostsWithSkipLockedOracle(
      EntityManager entityManager, int lockCount, int maxResults, Integer maxCount) {
        ...
        entityManager
            ...
            .setLockOptions(
                new LockOptions(LockMode.PESSIMISTIC_WRITE)
                    ...
                    // This is not really needed for this query but shows that you can control the
                    // follow-on locking mechanism
  ...
  private int pendingPostCount(EntityManager entityManager) {
    int postCount =
        ((Number)
                entityManager
                    .createQuery("select count(*) from Post where status = :status")
                    .setParameter("status", PostStatus.PENDING)
                    .getSingleResult())
            .intValue();
