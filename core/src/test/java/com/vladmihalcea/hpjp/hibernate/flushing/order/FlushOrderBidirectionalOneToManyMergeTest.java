    ...
    doInJPA(
        entityManager -> {
          entityManager
              ...
              .addComment(
                  new PostComment().setReview("The book size is larger than usual.").setSlug("/2"))
              .addComment(new PostComment().setReview("Just half-way through.").setSlug("/3"))
              .addComment(
                  new PostComment().setReview("The book has over 450 pages.").setSlug("/4"));
        });
  }
  ...
  private void verifyResults() {
    doInJPA(
        entityManager -> {
          ...
          assertEquals(
              "The book size is larger than usual.", post.getComments().get(1).getReview());
