...
public class SingleTableTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {
      Board.class, Topic.class, Post.class, Announcement.class, TopicStatistics.class
    };
  }
  ...
  protected void afterInit() {
    doInJPA(
        entityManager -> {
          Board board = new Board().setId(1L).setName("Hibernate");
          ...
          entityManager.persist(new TopicStatistics().setTopic(post).incrementViews());

          entityManager.persist(new TopicStatistics().setTopic(announcement).incrementViews());
        });
  }
  ...
  public void testPolymorphicAssociation() {
    ...
    doInJPA(
        entityManager -> {
          List<TopicStatistics> statistics =
              entityManager
                  .createQuery(
                      """
                select s
