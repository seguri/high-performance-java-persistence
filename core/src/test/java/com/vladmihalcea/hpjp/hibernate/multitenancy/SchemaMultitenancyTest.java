...
public class SchemaMultitenancyTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {User.class, Post.class};
  }
  ...
  protected void additionalProperties(Properties properties) {
    ...
    properties.put(
        AvailableSettings.MULTI_TENANT_CONNECTION_PROVIDER, MultiTenantConnectionProvider.INSTANCE);
  }

  @Override
  public void afterInit() {
    PGSimpleDataSource defaultDataSource =
        (PGSimpleDataSource) database().dataSourceProvider().dataSource();
    addTenantConnectionProvider(
        TenantContext.DEFAULT_TENANT_IDENTIFIER, defaultDataSource, propertiesMap());
              ...
              .doWork(
                  connection -> {
                    try (Statement statement = connection.createStatement()) {
                      ...
                      statement.executeUpdate(
                          String.format("SET search_path TO %s,public", schemaName));

                      statement.executeUpdate(
                          "create sequence hibernate_sequence start 1 increment 1");
                      statement.executeUpdate(
                          "create table posts (id int8 not null, created_on timestamp, title varchar(255), user_id int8, primary key (id))");
                      statement.executeUpdate(
                          "create table users (id int8 not null, registered_on timestamp, firstName varchar(255), lastName varchar(255), primary key (id))");
                      statement.executeUpdate(
                          "alter table if exists posts add constraint fk_user_id foreign key (user_id) references users");
  ...
  private void addTenantConnectionProvider(
      String tenantId, DataSource tenantDataSource, Map<String, Object> properties) {
