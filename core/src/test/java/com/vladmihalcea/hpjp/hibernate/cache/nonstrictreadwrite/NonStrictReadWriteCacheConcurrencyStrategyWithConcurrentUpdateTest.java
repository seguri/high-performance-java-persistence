...
/**
 * NonStrictReadWriteCacheConcurrencyStrategyWithConcurrentUpdateTest - Test to check
 * CacheConcurrencyStrategy.NONSTRICT_READ_WRITE
 *
 * @author Vlad Mihalcea
 */
public class NonStrictReadWriteCacheConcurrencyStrategyWithConcurrentUpdateTest
  ...
  private class BobTransaction extends EmptyInterceptor {
    ...
    public void beforeTransactionCompletion(Transaction tx) {
      if (applyInterceptor.get()) {
        ...
        assertFalse(sessionFactory().getCache().containsEntity(Repository.class, 1L));
        executeSync(
            () -> {
              EntityManager _entityManager = entityManagerFactory().createEntityManager();
              Repository repository = _entityManager.find(Repository.class, 1L);
              LOGGER.info("Cached Repository from Bob's transaction {}", repository);
              _entityManager.close();
              endLatch.countDown();
            });
        assertTrue(sessionFactory().getCache().containsEntity(Repository.class, 1L));
      }
    }
  }
    ...
    doInJPA(
        entityManager -> {
          ...
          entityManager
              ...
              .buildLockRequest(new LockOptions().setLockMode(LockMode.OPTIMISTIC))
              .lock(repository);
        });
    doInJPA(
        entityManager -> {
  ...
  public static class Repository {
    ...
    @jakarta.persistence.Version private short version;
