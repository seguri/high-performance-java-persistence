...
public class PostLegacyTest extends AbstractTest {
  ...
  public void afterInit() {
    doInJPA(
        entityManager -> {
          ...
          post.setCreatedOn(LocalDateTime.of(2020, 11, 2, 12, 0, 0));
  ...
  public void testRecord() {
    doInJPA(
        entityManager -> {
          ...
          assertEquals(1L, postInfo.getId().longValue());

          assertEquals("High-Performance Java Persistence", postInfo.getTitle());

          assertEquals(
              LocalDateTime.of(2016, 11, 2, 12, 0, 0), postInfo.getAuditInfo().getCreatedOn());

          assertEquals("Vlad Mihalcea", postInfo.getAuditInfo().getCreatedBy());
  ...
  public static class PostInfo {
    ...
    public PostInfo(Long id, String title, AuditInfo auditInfo) {
      this.id = id;
      this.title = title;
