...
public class BlazePersistenceBulkUpdateDeleteTest extends AbstractTest {
  ...
  public void testBulk() {
    doInJPA(
        entityManager -> {
          ...
          entityManager.persist(new Post().setId(2L).setTitle("Spam title"));

          entityManager.persist(new Post().setId(3L).setMessage("Spam message"));
  ...
  public <T extends PostModerate> int flagSpam(
      EntityManager entityManager, Class<T> postModerateClass) {

    UpdateCriteriaBuilder<T> builder =
        cbf.update(entityManager, postModerateClass)
    ...
    if (Post.class.isAssignableFrom(postModerateClass)) {
      ...
      builder.where(lower(PostComment_.MESSAGE)).like().value(spamToken).noEscape();
    }
  ...
  public <T extends PostModerate> int deleteSpam(
    ...
    return cbf.delete(entityManager, postModerateClass)
        .where(PostModerate_.STATUS)
        .eq(PostStatus.SPAM)
        .where(PostModerate_.UPDATED_ON)
        .le(
            Timestamp.valueOf(
                LocalDateTime.now()
                    .minusDays((Post.class.isAssignableFrom(postModerateClass)) ? 7 : 3)))
        .executeUpdate();
  }
