...
public class QueryCacheTest extends AbstractTest {
        ...
        entityManager -> {
          entityManager.persist(
              new Post()
                  ...
                  .addComment(new PostComment().setId(1L).setReview("JDBC part review")));
        });
  }
  ...
  private List<PostCommentSummary> getPostCommentSummaryByPost(
      EntityManager entityManager, Long postId) {
    return entityManager
        .createQuery(
            """
            select new
  ...
  public void test2ndLevelCacheWithQueryInvalidation() {
    doInJPA(
        entityManager -> {
          ...
          Post post = entityManager.find(Post.class, 1L);
          post.addComment(new PostComment().setId(2L).setReview("JDBC part review"));
          entityManager.flush();
