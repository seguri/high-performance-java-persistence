...
public class CriteriaAPIMemberOfTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class[] {CalendarEvent.class};
  }
  ...
  public void testFind() {
    doInJPA(
        entityManager -> {
          ...
          List<MailingCode> mailingCodes =
              Arrays.asList(new MailingCode(1), new MailingCode(2), new MailingCode(3));
          ...
          for (MailingCode mailingCode : mailingCodes) {
            predicate =
                builder.and(predicate, builder.isMember(mailingCode.getId(), mailingCodesPath));
          }
  ...
  public static class CalendarEvent implements Serializable {
    ...
    @ElementCollection private final List<Integer> mailingCodes = new ArrayList<>();
  }
