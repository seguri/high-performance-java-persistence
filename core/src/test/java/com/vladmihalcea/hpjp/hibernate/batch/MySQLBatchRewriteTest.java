...
public class MySQLBatchRewriteTest extends AbstractTest {
                  ...
                  connection -> {
                    try (Statement statement = connection.createStatement()) {
                      ...
                      for (long id = 1; id <= 10; id++) {
                        statement.addBatch(String.format(INSERT, id));
                      }
  ...
  private void insertPosts() {
    doInJPA(
        entityManager -> {
          for (long i = 1; i <= 10; i++) {
            entityManager.persist(new Post().setId(i).setTitle(String.format("Post no. %d", i)));
          }
        });
  }
          ...
          for (long i = 1; i <= 3; i++) {
            entityManager.persist(
                new Post()
                    ...
                    .addComment(new PostComment().setId(i).setReview("Good")));
          }
        });
  }
