...
public class SQLStatementCountValidatorTest extends AbstractTest {
  ...
  public void afterInit() {
    doInJPA(
        entityManager -> {
          Post post1 = new Post().setId(1L).setTitle("Post one");

          entityManager.persist(post1);

          entityManager.persist(new PostComment().setId(1L).setReview("Good").setPost(post1));

          Post post2 = new Post().setId(2L).setTitle("Post two");

          entityManager.persist(post2);

          entityManager.persist(new PostComment().setId(2L).setReview("Excellent").setPost(post2));
        });
  }
