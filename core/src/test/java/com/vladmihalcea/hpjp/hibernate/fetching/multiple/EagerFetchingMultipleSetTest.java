...
public class EagerFetchingMultipleSetTest extends AbstractPostgreSQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Post.class, PostComment.class, Tag.class};
  }
    ...
    doInJPA(
        entityManager -> {
          ...
          for (long i = 1; i <= TAG_COUNT; i++) {
            Tag tag = new Tag().setId(i).setName(String.format("Tag nr. %d", i + 1));
          ...
          for (long postId = 1; postId <= POST_COUNT; postId++) {
            Post post = new Post().setId(postId).setTitle(String.format("Post nr. %d", postId));
  ...
  public void testFindWithJoinFetchQuery() {
    doInJPA(
        entityManager -> {
          ...
          List<Tuple> posts =
              entityManager
                  .createNativeQuery(SQLExtractor.from(jpqlQuery), Tuple.class)
                  .setParameter(1, 1L)
                  .setParameter(2, 50L)
                  .getResultList();
