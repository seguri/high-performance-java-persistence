...
public class NestedLoopsSqlTest extends AbstractPostgreSQLIntegrationTest {
   ...
   * post ----
   *
   * <p>| id | title | |----|------------| | 1 | Post no. 1 | | 2 | Post no. 2 | | .. | .. |
   *
   * <p>post_comment -------------
   *
   * <p>| id | review | post_id | |----|---------------|---------| | 1 | Comment no. 1 | 1 | | 2 |
   * Comment no. 2 | 1 | | .. | .. | .. |
   */
  @Override
    ...
    doInJPA(
        entityManager -> {
          ...
          for (long postId = 1; postId <= postCount; postId++) {
            Post post = new Post().setId(postId).setTitle(String.format("Post no. %d", postId));
   ...
   *
   * <p>EXPLAIN ANALYZE SELECT p.id AS post_id, p.title AS post_title, pc.review AS review FROM post
   * p INNER JOIN post_comment pc ON pc.post_id = p.id WHERE p.id BETWEEN 1 AND 10
   */
  @Test
