...
public class CriteriaAPITest extends AbstractPostgreSQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class[] {Post.class, PostComment.class, Tag.class, PostDetails.class};
  }
        ...
        entityManager -> {
          Post post =
              new Post()
                  ...
                  .setDetails(
                      new PostDetails().setCreatedOn(new Date()).setCreatedBy("Vlad Mihalcea"));

          entityManager.persist(post);

          for (long i = 0; i < 5; i++) {
            post.addComment(new PostComment().setId(i + 1).setReview("Great"));
          }
        });
  }
  ...
  public void testFilterChildWithoutMetamodel() {
    doInJPA(
        entityManager -> {
          ...
          query.where(builder.equal(post.get("title"), "High-Performance Java Persistence"));
  ...
  public void testFetchObjectArray() {
    doInJPA(
        entityManager -> {
          ...
          criteria.where(
              builder.like(postJoin.get(Post_.title), "High-Performance Java Persistence"));
  ...
  public void testFetchObjectArrayToDTO() {
    doInJPA(
        entityManager -> {
          ...
          query.where(
              builder.and(
                  builder.like(post.get(Post_.title), "%Java Persistence%"),
                  builder.equal(
                      post.get(Post_.details).get(PostDetails_.CREATED_BY), "Vlad Mihalcea")));

          List<PostCommentSummary> comments =
              entityManager
                  .createQuery(query)
                  .unwrap(Query.class)
