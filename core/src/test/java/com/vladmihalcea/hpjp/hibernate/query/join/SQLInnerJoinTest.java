...
public class SQLInnerJoinTest extends AbstractTest {
   ...
   * <p>post_comment -------------
   *
   * <p>| id | review | p.id | |----|-----------|---------| | 1 | Good | 1 | | 2 | Excellent | 1 | |
   * 3 | Awesome | 2 |
   */
  @Override
  public void afterInit() {
    doInJPA(
        entityManager -> {
          Post post1 = new Post().setId(1L).setTitle("Java");

          entityManager.persist(post1);

          Post post2 = new Post().setId(2L).setTitle("Hibernate");

          entityManager.persist(post2);

          Post post3 = new Post().setId(3L).setTitle("JPA");

          entityManager.persist(post3);

          entityManager.persist(new PostComment().setId(1L).setReview("Good").setPost(post1));

          entityManager.persist(new PostComment().setId(2L).setReview("Excellent").setPost(post1));

          entityManager.persist(new PostComment().setId(3L).setReview("Awesome").setPost(post2));
        });
  }
   ...
   *
   * <p>SELECT p.id AS "p.id", pc.id AS "pc.id" FROM post p INNER JOIN post_comment pc ON 1 = 0
   *
   * <p>| p.id | pc.id | |---------|------------|
   */
  @Test
   ...
   *
   * <p>SELECT p.id AS "p.id", pc.id AS "pc.id" FROM post p INNER JOIN post_comment pc ON 1 = 1
   * ORDER BY p.id, pc.id
   *
   * <p>| p.id | pc.id | |---------|------------| | 1 | 1 | | 1 | 2 | | 1 | 3 | | 2 | 1 | | 2 | 2 |
   * | 2 | 3 | | 3 | 1 | | 3 | 2 | | 3 | 3 |
   */
  @Test
   ...
   *
   * <p>SELECT p.id AS "p.id", pc.post_id AS "pc.post_id", pc.id AS "pc.id", p.title AS "p.title",
   * pc.review AS "pc.review" FROM post p INNER JOIN post_comment pc ON pc.post_id = p.id ORDER BY
   * p.id, pc.id
   *
   * <p>| p.id | pc.post_id | pc.id | p.title | pc.review |
   * |---------|------------|------------|------------|-----------| | 1 | 1 | 1 | Java | Good | | 1
   * | 1 | 2 | Java | Excellent | | 2 | 2 | 3 | Hibernate | Awesome |
   */
  @Test
   ...
   *
   * <p>SELECT p.id AS "p.id", pc.post_id AS "pc.post_id", pc.id AS "pc.id", p.title AS "p.title",
   * pc.review AS "pc.review" FROM post p, post_comment pc WHERE pc.post_id = p.id ORDER BY p.id,
   * pc.id
   *
   * <p>| p.id | pc.post_id | pc.id | p.title | pc.review |
   * |---------|------------|------------|------------|-----------| | 1 | 1 | 1 | Java | Good | | 1
