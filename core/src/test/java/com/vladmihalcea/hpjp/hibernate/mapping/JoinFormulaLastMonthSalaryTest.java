...
public class JoinFormulaLastMonthSalaryTest extends AbstractPostgreSQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Employee.class, Salary.class};
  }
  ...
  public void test() {
    doInJPA(
        entityManager -> {
          assertEquals(
              Long.valueOf(1L),
              entityManager.find(Salary.class, 3L).getPreviousMonthSalary().getId());

          assertEquals(
              Long.valueOf(2L),
              entityManager.find(Salary.class, 4L).getPreviousMonthSalary().getId());

          assertEquals(
              Long.valueOf(3L),
              entityManager.find(Salary.class, 5L).getPreviousMonthSalary().getId());

          assertEquals(
              Long.valueOf(4L),
              entityManager.find(Salary.class, 6L).getPreviousMonthSalary().getId());

          assertEquals(
              Long.valueOf(5L),
              entityManager.find(Salary.class, 7L).getPreviousMonthSalary().getId());

          assertEquals(
              Long.valueOf(6L),
              entityManager.find(Salary.class, 8L).getPreviousMonthSalary().getId());
        });
  }
