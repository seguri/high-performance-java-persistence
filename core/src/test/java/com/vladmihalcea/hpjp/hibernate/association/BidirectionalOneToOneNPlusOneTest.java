...
public class BidirectionalOneToOneNPlusOneTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Post.class, PostDetails.class, PostSummary.class};
  }
  ...
  protected void additionalProperties(Properties properties) {
    properties.put(
        ...
        (IntegratorProvider)
            () -> Collections.singletonList(new ClassImportIntegrator(List.of(PostDTO.class))));
  }
  ...
  public void testFetchPostAndDetailsProjection() {
    doInJPA(
        entityManager -> {
          for (int i = 1; i <= 100; i++) {
            entityManager.persist(new Post(String.format("Post nr. %d", 100 + i)));
          }
        });
