...
public class WithCTETest extends AbstractTest {
   ...
   *
   * <p>WITH p_pc AS ( SELECT p.id AS post_id, p.title AS post_title, pc.id AS comment_id, pc.review
   * AS comment_review, COUNT(post_id) OVER(PARTITION BY post_id) AS comment_count FROM post p LEFT
   * JOIN post_comment pc ON p.id = pc.post_id WHERE p.title LIKE 'SQL%' ), p_pc_r AS ( SELECT
   * post_id, post_title, comment_id, comment_review, DENSE_RANK() OVER (ORDER BY p_pc.comment_count
   * DESC) AS ranking FROM p_pc ) SELECT * FROM p_pc_r WHERE p_pc_r.ranking <= 2 ORDER BY post_id,
   * comment_id
   *
