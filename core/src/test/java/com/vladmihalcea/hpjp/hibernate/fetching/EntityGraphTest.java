...
public class EntityGraphTest extends AbstractPostgreSQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {
      Post.class, PostDetails.class, PostComment.class, PostCommentDetails.class
    };
  }
          ...
          for (long i = 1; i <= 3; i++) {
            entityManager.persist(
                new Post()
                    ...
                    .setDetails(
                        new PostDetails().setCreatedBy("Vlad Mihalcea").setCreatedOn(new Date()))
                    .addComment(
                        new PostComment()
                            .setId(commentId++)
  ...
  public void testFindWithNamedEntityFetchGraph() {
    PostComment comment =
        doInJPA(
            entityManager -> {
              EntityGraph<PostComment> postCommentGraph =
  ...
  public void testFindUsingNestedEntityGraph() {
    PostCommentDetails commentDetails =
        doInJPA(
            entityManager -> {
              EntityGraph<PostCommentDetails> commentDetailsGraph =
                  entityManager.createEntityGraph(PostCommentDetails.class);
              ...
              return entityManager.find(
                  ...
                  1L,
                  Collections.singletonMap("jakarta.persistence.loadgraph", commentDetailsGraph));
            });
        ...
        doInJPA(
            entityManager -> {
              return entityManager.find(
                  ...
                  1L,
                  Collections.singletonMap(
                      "jakarta.persistence.fetchgraph", entityManager.getEntityGraph("Post.all")));
            });
  ...
  @Entity(name = "Post")
  @Table(name = "post")
  @NamedEntityGraph(name = "Post.details", attributeNodes = @NamedAttributeNode("details"))
  @NamedEntityGraph(
      name = "Post.all",
      attributeNodes = {
