...
public class ConnectionStatisticsTest extends AbstractTest {
  ...
  protected void additionalProperties(Properties properties) {
    ...
    properties.put(StatisticsInitiator.STATS_BUILDER, TransactionStatisticsFactory.class.getName());
  }
    ...
    for (long i = 1; i <= iterations; i++) {
      ...
      doInJPA(
          entityManager -> {
            Post post = new Post();
            post.setTitle(
                String.format("High-Performance Java Persistence, Part %d", currentIteration));
