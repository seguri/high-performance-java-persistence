...
public class GroupByMapTest extends AbstractPostgreSQLIntegrationTest {
  ...
  public void testGroupByStreamCollector() {
    doInJPA(
        entityManager -> {
          Map<Integer, Integer> postCountByYearMap =
              entityManager
                  .createQuery(
                      "select "
  ...
  public void testGroupByResultTransformer() {
    doInJPA(
        entityManager -> {
          Map<Number, Number> postCountByYearMap =
              (Map<Number, Number>)
                  entityManager
                      .createQuery(
                          "select "
                      ...
                      .setResultTransformer(new MapResultTransformer<Number, Number>())
                      .getSingleResult();
  ...
  public void testGroupByIdStreamCollector() {
    doInJPA(
        entityManager -> {
          ...
          assertEquals(
              "Hypersistence Optimizer has been released!", postByIdMap.get(5L).getTitle());
        });
  }
