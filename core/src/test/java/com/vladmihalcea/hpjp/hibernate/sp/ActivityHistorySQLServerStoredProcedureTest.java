...
public class ActivityHistorySQLServerStoredProcedureTest extends AbstractSQLServerIntegrationTest {
  ...
  private int insertProcInst(EntityManager entityManager, int procId, Integer parentProcId) {
    entityManager
        ...
        .setParameter(
            "super_process_instance_id_",
            parentProcId != null ? String.valueOf(parentProcId) : null)
        .setParameter(
            "start_time_",
            Timestamp.valueOf(LocalDate.of(2020, 11, 25).atStartOfDay().plusHours(procId)))
        .setParameter(
            "end_time_",
            Timestamp.valueOf(LocalDate.of(2020, 12, 25).atStartOfDay().plusHours(procId)))
        .executeUpdate();
    ...
    try (PreparedStatement preparedStatement =
      ...
      for (int j = 1; j <= ACT_HI_ACTINST_PER_PROC_COUNT; j++) {
        ...
        preparedStatement.setTimestamp(
            index++,
            Timestamp.valueOf(
                LocalDate.of(2020, 11, 25).atStartOfDay().plusHours(procId).plusMinutes(j)));
  ...
  private int insertTaskInst(EntityManager entityManager, int procId, Integer parentTaskId) {
    ...
    entityManager
        ...
        .setParameter(
            "start_time_",
            Timestamp.valueOf(
                LocalDate.of(2020, 11, 25).atStartOfDay().plusHours(procId).plusMinutes(taskId)))
        .executeUpdate();
  ...
  private void insertVarInsts(Connection connection, int procId, int taskId, boolean subTask)
      throws SQLException {
    try (PreparedStatement varInstPreparedStatement =
            connection.prepareStatement(
                """
            ...
            """);
        PreparedStatement byteArrayPreparedStatement =
            connection.prepareStatement(
                """
      ...
      for (int i = 1; i <= ACT_HI_VARINST_PER_TASK_COUNT; i++) {
        ...
        varInstPreparedStatement.setTimestamp(
            index++,
            Timestamp.valueOf(
  ...
  private void insertDetails(Connection connection, int procId) throws SQLException {
    try (PreparedStatement detailPreparedStatement =
            connection.prepareStatement(
                """
      ...
      for (int i = 1; i <= ACT_HI_DETAIL_PER_PROC_COUNT; i++) {
        ...
        detailPreparedStatement.setTimestamp(
            index++,
            Timestamp.valueOf(LocalDate.of(2020, 11, 25).atStartOfDay().plusHours(procId)));
        ...
        index = 1;
        byteArrayPreparedStatement.setString(index++, String.valueOf(byteArrayId));
        byteArrayPreparedStatement.setString(
            index++, String.format("Proc Detail Byte Array: %d", id));
  ...
  private void insertDetails(Connection connection, int procId, int taskId, boolean subTask)
      throws SQLException {
    try (PreparedStatement detailPreparedStatement =
            connection.prepareStatement(
                """
      ...
      for (int i = 1; i <= ACT_HI_DETAIL_PER_TASK_COUNT; i++) {
        ...
        byteArrayPreparedStatement.setString(index++, String.valueOf(byteArrayId));
        byteArrayPreparedStatement.setString(
            index++, String.format("Task Detail Byte Array: %d", id));
  ...
  private void insertComments(Connection connection, int procId, int taskId, boolean subTask)
      throws SQLException {
    try (PreparedStatement preparedStatement =
        connection.prepareStatement(
            """
  ...
  private void insertComments(EntityManager entityManager, int procId) {
    ...
    entityManager
        ...
        .setParameter(
            "time",
            Timestamp.valueOf(
                LocalDate.of(2020, 11, 25).atStartOfDay().plusHours(procId).plusMinutes(id)))
        .executeUpdate();
  }

  private void insertAttachments(Connection connection, int procId, int taskId, boolean subTask)
      throws SQLException {
    try (PreparedStatement attachmentPreparedStatement =
            connection.prepareStatement(
                """
      ...
      for (int i = 1; i <= ACT_HI_ATTACHMENT_PER_TASK_COUNT; i++) {
        ...
        attachmentPreparedStatement.setTimestamp(
            index++,
            Timestamp.valueOf(
                LocalDate.of(2020, 11, 25)
                    .atStartOfDay()
  ...
  private void insertIdentityLinks(Connection connection, int procId, int taskId, boolean subTask)
      throws SQLException {
    try (PreparedStatement preparedStatement =
        connection.prepareStatement(
            """
  ...
  public void testDeleteActivityHistory() {
    ...
    try (Connection connection = dataSourceProvider().dataSource().getConnection()) {
      deleteActivityHistoryBeforeDate(connection, Timestamp.valueOf(LocalDateTime.now()), 10);
    ...
    doInJPA(
        entityManager -> {
          assertEquals(
              ...
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_PROCINST")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_ACTINST")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_TASKINST")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_GE_BYTEARRAY")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_VARINST")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_DETAIL")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_COMMENT")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_ATTACHMENT")
                          .getSingleResult())
                  .intValue());
          assertEquals(
              0,
              ((Number)
                      entityManager
                          .createNativeQuery("SELECT COUNT(*) FROM ACT_HI_IDENTITYLINK")
                          .getSingleResult())
                  .intValue());
        });
  }

  private int deleteActivityHistoryBeforeDate(
      Connection connection, Timestamp olderThanTimestamp, int batchSize) {
    long startNanos = System.nanoTime();
    try (CallableStatement sp =
        connection.prepareCall("{ call usp_DeleteActivityHistory(?, ?, ?) }")) {
