...
public class PaginationTest extends AbstractTest {
  ...
  protected void additionalProperties(Properties properties) {
    properties.setProperty(
        AvailableSettings.FAIL_ON_PAGINATION_OVER_COLLECTION_FETCH, Boolean.FALSE.toString());
  }
  ...
  public void afterInit() {
    doInJPA(
        entityManager -> {
          LocalDateTime timestamp = LocalDateTime.of(2018, 10, 9, 12, 0, 0, 0);
  ...
  public void testDTO() {
    doInJPA(
        entityManager -> {
          ...
          assertEquals(
              "High-Performance Java Persistence - Chapter 1", summaries.get(0).getTitle());
          assertEquals("Comment nr. 1 - A must-read!", summaries.get(0).getReview());

          assertEquals(
              "High-Performance Java Persistence - Chapter 2", summaries.get(9).getTitle());
    ...
    doInJPA(
        entityManager -> {
          ...
          for (int i = 0; i < COMMENT_COUNT - 1; i++) {
            ...
            assertEquals(
                String.format("Comment nr. %d - A must-read!", i + 1), postComment1.getReview());
          }
        });
  }
        ...
        entityManager -> {
          entityManager.persist(
              new Post()
                  ...
                  .setCreatedOn(Timestamp.valueOf(LocalDateTime.of(2018, 10, 8, 12, 0, 0, 0))));
        });

    List<Post> posts =
        doInJPA(
            entityManager -> {
              DistinctPostResultTransformer resultTransformer =
