...
public class MySQLMDCLoggingTest extends AbstractMySQLIntegrationTest {
  ...
  public void testWithMDC() {
    doInJPA(
        entityManager -> {
          ...
          try (MDC.MDCCloseable closable =
              MDC.putCloseable(
                  "txId", String.format(" TxId: [%s]", transactionId(entityManager)))) {
            executeSync(
                () -> {
                  doInJPA(
                      _entityManager -> {
                        LOGGER.info("Acquire lock so that the TxId is assigned");
                        _entityManager.persist(new Post().setId(2L).setTitle("New Post!"));
                        _entityManager.flush();
                        sleep(100);

                        try (MDC.MDCCloseable _closable =
                            MDC.putCloseable(
                                "txId",
  ...
  protected String threadId(EntityManager entityManager) {
    return String.valueOf(
        entityManager.createNativeQuery("SELECT CONNECTION_ID()").getSingleResult());
  }
