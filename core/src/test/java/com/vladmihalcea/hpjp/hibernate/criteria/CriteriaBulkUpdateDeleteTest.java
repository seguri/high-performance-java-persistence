...
public class CriteriaBulkUpdateDeleteTest extends AbstractTest {
  ...
  public <T extends PostModerate> int flagSpam(
    ...
    if (Post.class.isAssignableFrom(postModerateClass)) {
      filterPredicate =
          builder.or(filterPredicate, builder.like(builder.lower(root.get("title")), "%spam%"));
    }
    ...
    delete.where(
        builder.and(
            ...
            builder.lessThanOrEqualTo(
                root.get("updatedOn"),
                Timestamp.valueOf(LocalDateTime.now().minusDays(daysValidityThreshold)))));
  ...
  @MappedSuperclass
  public abstract static class PostModerate<T extends PostModerate> {
