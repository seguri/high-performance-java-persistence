...
public class CollectionCacheTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Repository.class, Commit.class};
  }
  ...
  public void testConsistencyWhenHQLUpdating() {
    ...
    doInJPA(
        entityManager -> {
          entityManager.createQuery("update Commit c " + "set c.review = true ").executeUpdate();
        });
    doInJPA(
        entityManager -> {
          Repository repository = (Repository) entityManager.find(Repository.class, 1L);
  ...
  public void testConsistencyWhenSQLUpdating() {
    ...
    doInJPA(
        entityManager -> {
          entityManager
              .createNativeQuery("update Commit c " + "set c.review = true ")
  ...
  public void testConsistencyWhenManuallySQLUpdating() {
    ...
    final Repository repository =
        doInJPA(
            entityManager -> {
              Repository _repository = (Repository) entityManager.find(Repository.class, 1L);
    ...
    doInJPA(
        entityManager -> {
              ...
              .doWork(
                  connection -> {
                    try (PreparedStatement statement =
                        connection.prepareStatement("update Commit c " + "set c.review = true ")) {
          ...
          entityManager
              ...
              .getCache()
              .evictCollectionData(Repository.class.getName() + ".commits", repository.getId());
        });
    doInJPA(
        entityManager -> {
          Repository _repository = entityManager.find(Repository.class, 1L);
          for (Commit commit : _repository.getCommits()) {
            assertTrue(commit.review);
          }
        });
