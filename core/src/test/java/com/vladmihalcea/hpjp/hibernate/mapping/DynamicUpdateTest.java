...
public class DynamicUpdateTest extends AbstractTest {
  ...
  public void test() {
    ...
    doInJPA(
        entityManager -> {
          ...
          entityManager.persist(new Post().setId(2L).setTitle("Java Persistence with Hibernate"));
        });
  ...
  public void testBatch() {
    ...
    doInJPA(
        entityManager -> {
          for (long i = 1; i <= 5; i++) {
            entityManager.persist(new Post().setId(i).setTitle(String.format("Post %s", i)));
          }
        });
