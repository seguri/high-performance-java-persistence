...
public class DerivedTableTest extends AbstractTest {
   ...
   * post ----
   *
   * <p>| id | title | |----|----------| | 1 | SQL:2016 | | 2 | SQL:2011 | | 3 | SQL:2008 | | 4 |
   * JPA 3.0 | | 5 | JPA 2.2 | | 6 | JPA 2.1 | | 7 | JPA 2.0 | | 8 | JPA 1.0 |
   *
   * <p>post_comment -------------
   *
   * <p>| id | review | post_id | |----|------------------------|---------| | 1 | SQL:2016 is great!
   * | 1 | | 2 | SQL:2016 is excellent! | 1 | | 3 | SQL:2016 is awesome! | 1 | | 4 | SQL:2011 is
   * great! | 2 | | 5 | SQL:2011 is excellent! | 2 | | 6 | SQL:2008 is great! | 3 |
   */
  @Override
  public void afterInit() {
    doInJPA(
        entityManager -> {
          short[] latestSQLStandards = {2016, 2011, 2008};

          String[] comments = {"great", "excellent", "awesome"};
          ...
          for (int i = 0; i < latestSQLStandards.length; i++) {
            ...
            Post post = new Post().setId(post_id++).setTitle(String.format("SQL:%d", sqlStandard));
          ...
          entityManager.persist(new Post().setId(post_id++).setTitle("JPA 3.0"));
          entityManager.persist(new Post().setId(post_id++).setTitle("JPA 2.2"));
          entityManager.persist(new Post().setId(post_id++).setTitle("JPA 2.1"));
          entityManager.persist(new Post().setId(post_id++).setTitle("JPA 2.0"));
          entityManager.persist(new Post().setId(post_id++).setTitle("JPA 1.0"));
        });
  }
   ...
   *
   * <p>SELECT * FROM ( SELECT post_id, post_title, comment_id, comment_review, DENSE_RANK() OVER
   * (ORDER BY p_pc.comment_count DESC) AS ranking FROM ( SELECT p.id AS post_id, p.title AS
   * post_title, pc.id AS comment_id, pc.review AS comment_review, COUNT(post_id) OVER(PARTITION BY
   * post_id) AS comment_count FROM post p LEFT JOIN post_comment pc ON p.id = pc.post_id WHERE
   * p.title LIKE 'SQL%' ) p_pc ) p_pc_r WHERE p_pc_r.ranking <= 2 ORDER BY post_id, comment_id
   *
   * <p>| post_id | post_title | comment_id | comment_review | ranking |
   * |---------|------------|------------|------------------------|---------| | 1 | SQL:2016 | 1 |
   * SQL:2016 is great! | 1 | | 1 | SQL:2016 | 2 | SQL:2016 is excellent! | 1 | | 1 | SQL:2016 | 3 |
   * SQL:2016 is awesome! | 1 | | 2 | SQL:2011 | 4 | SQL:2011 is great! | 2 | | 2 | SQL:2011 | 5 |
   * SQL:2011 is excellent! | 2 |
   */
  @Test
    ...
    List<Tuple> tuples =
        doInJPA(
            entityManager -> {
              ...
              return query.setParameter("title", "SQL%").getResultList();
            });
