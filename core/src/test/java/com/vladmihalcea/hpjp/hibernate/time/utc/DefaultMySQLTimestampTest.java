    ...
    try {
      ...
      doInJPA(
          entityManager -> {
            ...
            book.setCreatedOn(
                new Timestamp(
                    ZonedDateTime.of(2016, 8, 25, 11, 23, 46, 0, ZoneId.of("UTC"))
                        .toInstant()
                        .toEpochMilli()));
            ...
            session.doWork(
                connection -> {
                  try (Statement st = connection.createStatement()) {
                    try (ResultSet rs =
                        st.executeQuery(
                            "SELECT DATE_FORMAT(created_on, '%Y-%m-%d %H:%i:%s') " + "FROM book")) {
                      while (rs.next()) {
                        String timestamp = rs.getString(1);
                        if (!expectedServerTimestamp().equals(timestamp)) {
                          LOGGER.error(
                              "Expected {}, but got {}", expectedServerTimestamp(), timestamp);
                        }
                      }
                    }
                  }
