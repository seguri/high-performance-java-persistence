  ...
  public void afterInit() {
    doInJPA(
        entityManager -> {
          ...
          String[] reviews =
              new String[] {
                "Amazing", "Great", "Excellent", "Highly recommended", "Simply the best"
              };

          long postId = 0;

          Post post1 = new Post().setId(++postId).setTitle("High-Performance Java Persistence");

          Post post2 = new Post().setId(++postId).setTitle("High-Performance SQL");
  ...
  public void testExistsCriteriaAPI() {
    doInJPA(
        entityManager -> {
          ...
          List<Post> posts =
              entityManager.createQuery(query).setParameter(minScore, 10).getResultList();
        ...
        entityManager -> {
          ...
          List<Post> posts =
              cbf.create(entityManager, Post.class)
                  ...
                  .select("1")
                  .where(PostComment_.POST)
                  .eqExpression(POST_ALIAS)
                  .where(PostComment_.SCORE)
                  .gtExpression(":minScore")
                  .end()
                  .select(POST_ALIAS)
                  .setParameter("minScore", 10)
                  .getResultList();
