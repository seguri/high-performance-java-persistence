...
public class JPACalculatedPostLoadTest extends AbstractPostgreSQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Account.class, User.class};
  }
  ...
  public static class Account {
    ...
    @Transient private double dollars;

    @Transient private long interestCents;

    @Transient private double interestDollars;

    public Account() {}

    public Account(
        Long id, User owner, String iban, long cents, double interestRate, Timestamp createdOn) {
      this.id = id;
      this.owner = owner;
      this.iban = iban;
      this.cents = cents;
    ...
    private void postLoad() {
      ...
      this.interestCents =
          BigDecimal.valueOf(interestUnrounded).setScale(0, BigDecimal.ROUND_HALF_EVEN).longValue();
