  ...
  public void test() {
    ...
    doInJPA(
        entityManager -> {
          ...
          for (int offset = 0; offset < 10; offset++) {
            Post post = new Post();
            post.setTitle(String.format("High-Performance Java Persistence, Review %d", offset));
            post.setCreatedOn(
                Date.from(startDate.plusDays(offset).atStartOfDay(ZoneId.of("UTC")).toInstant()));
    ...
    doInJPA(
        entityManager -> {
          List<Post> posts =
              entityManager
                  .createQuery("select p " + "from Post p " + "order by p.createdOn", Post.class)
                  .setMaxResults(5)
                  .getResultList();
