...
public class LazyToOneFalseTest extends AbstractTest {
  ...
  public void testLazyLoadingNoProxy() {
    final Post post = new Post().setId(1L).setTitle("High-Performance Java Persistence, 1st Part");

    doInJPA(
        entityManager -> {
          entityManager.persist(post);

          entityManager.persist(new PostComment().setId(1L).setReview("Amazing!").setPost(post));
        });
