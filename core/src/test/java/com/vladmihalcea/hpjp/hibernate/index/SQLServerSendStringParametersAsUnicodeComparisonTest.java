...
public class SQLServerSendStringParametersAsUnicodeComparisonTest extends AbstractTest {
  ...
  public SQLServerSendStringParametersAsUnicodeComparisonTest(
    ...
    doInJPA(
        entityManager -> {
          ...
          findByTitle(
              entityManager,
              String.format(
                  "High-Performance Java Persistence, part %d", random.nextLong(POST_COUNT)));
  ...
  @Entity(name = "Post")
  @Table(name = "Post", indexes = @Index(name = "IDX_Post_Title", columnList = "Title"))
