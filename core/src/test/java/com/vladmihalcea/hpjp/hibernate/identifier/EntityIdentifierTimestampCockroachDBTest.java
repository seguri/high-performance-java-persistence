                  ...
                  connection -> {
                    try (PreparedStatement preparedStatement =
                        connection.prepareStatement(
                            "INSERT INTO post (title, createdOn) " + "VALUES (?, ?)")) {
                      int index = 0;
                      preparedStatement.setString(++index, "High-Performance Java Persistence");
                      preparedStatement.setTimestamp(
                          ++index, new Timestamp(System.currentTimeMillis()));
