...
public class TsidTest {
  ...
  public void test() {
    ...
    for (int i = 0; i < 10; i++) {
      LOGGER.info("TSID numerical value: {}", TSID.fast().toLong());
    }
  }
      ...
      new Thread(
              () -> {
                TSID.Factory tsidFactory =
                    TsidUtils.getTsidFactory(nodeCount, threadId % nodeCount);
    ...
    for (int i = 0; i < threadCount; i++) {
      ...
      new Thread(
              () -> {
                ...
                TSID.Factory tsidFactory = builder.build();
