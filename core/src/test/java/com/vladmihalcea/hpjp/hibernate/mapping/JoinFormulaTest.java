...
public class JoinFormulaTest extends AbstractPostgreSQLIntegrationTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Language.class, Country.class, Account.class};
  }
        ...
        connection -> {
          try (Statement statement = connection.createStatement()) {
            statement.executeUpdate(
                "CREATE INDEX account_language_idx ON Account (REGEXP_REPLACE(locale, '(\\w+)_.*', '\\1'))");
            statement.executeUpdate(
                "CREATE INDEX account_country_idx ON Account (REGEXP_REPLACE(locale, '\\w+_(\\w+)[_]?', '\\1'))");
          }
        });
