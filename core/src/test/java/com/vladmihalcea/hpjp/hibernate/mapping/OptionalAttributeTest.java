...
public class OptionalAttributeTest extends AbstractTest {
  ...
  protected Class<?>[] entities() {
    return new Class<?>[] {Post.class, PostComment.class, Attachment.class};
  }
    ...
    doInJPA(
        entityManager -> {
          ...
          List<Attachment> attachments =
              comments.stream()
                  .map(pc -> pc.getAttachment().orElse(notAvailable))
                  .collect(Collectors.toList());

          attachments.forEach(
              attachment -> LOGGER.info("Attachment content {}", attachment.getContent()));
        });
  }
