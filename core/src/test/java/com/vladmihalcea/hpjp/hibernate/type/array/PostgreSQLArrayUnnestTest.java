...
public class PostgreSQLArrayUnnestTest extends AbstractPostgreSQLIntegrationTest {
  ...
  public void test() {
    doInJPA(
        entityManager -> {
          entityManager.persist(new Event().setT1(new int[] {1, 3, 6}).setT2(new int[] {8, 9}));

          entityManager.persist(new Event().setT1(new int[] {1, 2}).setT2(new int[] {8}));

          entityManager.persist(new Event().setT1(new int[] {6}).setT2(new int[] {8, 1}));
        });
