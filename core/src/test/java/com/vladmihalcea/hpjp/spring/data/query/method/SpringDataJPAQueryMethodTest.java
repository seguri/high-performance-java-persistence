...
public class SpringDataJPAQueryMethodTest {
  ...
  public void testFindByPostAndStatusOrderByCreatedOn() {
    ...
    List<PostComment> comments =
        postCommentRepository.findAllByPostAndStatusOrderByCreatedOn(
            post, PostComment.Status.PENDING);
    assertEquals(POST_COMMENT_COUNT, comments.size());
  }
  ...
  public void testFindByPostAndStatusAndReviewLikeOrderByCreatedOn() {
    ...
    List<PostComment> comments =
        postCommentRepository.findAllByPostAndStatusAndReviewLikeOrderByCreatedOn(
            post, PostComment.Status.PENDING, reviewPattern);
    ...
    int expectedCommentCount = 0;
    {
      List<PostComment> comments =
          postCommentRepository
              .findAllByPostAndStatusAndReviewLikeAndVotesGreaterThanEqualOrderByCreatedOn(
                  post, PostComment.Status.PENDING, reviewPattern, minVotes);

      expectedCommentCount = comments.size();
    }

    List<PostComment> comments =
        postCommentRepository.findAllByPostStatusReviewAndMinVotes(
            post, PostComment.Status.PENDING, reviewPattern, minVotes);
