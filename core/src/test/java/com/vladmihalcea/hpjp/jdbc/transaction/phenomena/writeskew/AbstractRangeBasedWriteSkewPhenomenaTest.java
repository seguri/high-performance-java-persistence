...
public abstract class AbstractRangeBasedWriteSkewPhenomenaTest
    extends AbstractDepartmentEmployeePhenomenaTest {

  protected AbstractRangeBasedWriteSkewPhenomenaTest(
      String isolationLevelName, int isolationLevel) {
  ...
  public void testWriteSkewAggregateWriteSkewAggregate() {
                        ...
                        bobConnection -> {
                          ...
                          try {
                            ...
                            try (PreparedStatement employeeStatement =
                                bobConnection.prepareStatement(insertEmployeeSql()); ) {
                              int employeeId = 4;
                              int index = 0;
    ...
    doInJDBC(
        aliceConnection -> {
          long salaryCount =
              selectColumn(
                      aliceConnection, sumEmployeeSalarySql(), Number.class, Duration.ofSeconds(1))
                  .longValue();
          if (99_000 != salaryCount) {
            LOGGER.info(
                "Isolation level {} allows Write Skew since the salary count is {} instead of 99000",
                isolationLevelName,
                salaryCount);
          } else {
            LOGGER.info(
                "Isolation level {} prevents Write Skew due to {}",
                isolationLevelName,
                preventedByLocking.get() ? "locking" : preventedByMVCC.get() ? "MVCC" : "unknown");
          }
        });
  }
    ...
    try {
      doInJDBC(
          aliceConnection -> {
            ...
            try (PreparedStatement employeeStatement =
                aliceConnection.prepareStatement(insertEmployeeSql()); ) {
              int employeeId = 5;
              int index = 0;
              employeeStatement.setLong(++index, 1);
    ...
    try {
      doInJDBC(
          aliceConnection -> {
            ...
            List<Number> salaries =
                selectColumnList(aliceConnection, allEmployeeSalarySql(), Number.class);
                  ...
                  () -> {
                    doInJDBC(
                        bobConnection -> {
                          prepareConnection(bobConnection);
                          try {
                            List<Number> _salaries =
                                selectColumnList(
                                    bobConnection, allEmployeeSalarySql(), Number.class);
                            assertEquals(
                                90_000, _salaries.stream().mapToInt(Number::intValue).sum());
