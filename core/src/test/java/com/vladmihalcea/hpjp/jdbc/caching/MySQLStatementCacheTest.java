...
public class MySQLStatementCacheTest extends DatabaseProviderIntegrationTest {
          ...
          _entityManager -> {
            executeWithTiming(
                ...
                (EntityManager entityManager) -> {
                  ...
                  comment.setReview(String.format("Post comment - %1$d", millis));
                });

            executeWithTiming(_entityManager, flushTimer, EntityManager::flush);
  ...
  private void executeWithTiming(
      EntityManager entityManager, Timer timer, Consumer<EntityManager> consumer) {
  ...
  private void printServerSidePreparedStatementCount(Connection connection) throws SQLException {
    try (Statement psCountStatement = connection.createStatement();
        ResultSet psCountResultSet =
            psCountStatement.executeQuery("SHOW SESSION STATUS LIKE 'Prepared_stmt_count'")) {
