...
/**
 * AbstractSequenceGeneratedKeysBatchPreparedStatementTest - Base class for testing JDBC
...
public abstract class AbstractSequenceGeneratedKeysBatchPreparedStatementTest extends AbstractTest {
  ...
  protected void batchInsert(Connection connection) throws SQLException {
    ...
    LOGGER.info(
        "{} Driver supportsGetGeneratedKeys: {}",
    ...
    try (PreparedStatement postStatement =
        connection.prepareStatement("INSERT INTO post (id, title, version) VALUES (?, ?, ?)")) {
    ...
    LOGGER.info(
        "{}.testInsert for {} using allocation size {} took {} millis",
  ...
  protected void createSequence(Connection connection) {
    try (Statement statement = connection.createStatement()) {
      statement.executeUpdate(
          String.format(
              "create sequence post_seq start with 1 increment by %d", getAllocationSize()));
    } catch (Exception ignore) {
    }
  }
}
