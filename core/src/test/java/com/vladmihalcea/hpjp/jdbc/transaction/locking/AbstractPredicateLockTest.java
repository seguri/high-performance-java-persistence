    ...
    doInHibernate(
        session -> {
          ...
          List<PostComment> comments =
              session
                  .createQuery(
                      "select c " + "from PostComment c " + "where c.post.id = :id",
                      PostComment.class)
                  .setParameter("id", 1L)
                ...
                try {
                  doInHibernate(
                      _session -> {
                        ...
                        _session.flush();
                        LOGGER.info(
                            "Insert {} prevented by explicit lock",
                ...
                try {
                  doInHibernate(
                      _session -> {
                        ...
                        LOGGER.info(
                            "Delete {} prevented by explicit lock",
                            prevented.get() ? "was" : "was not");
                        bobLatch.countDown();
                      });
                } catch (Exception e) {
                ...
                try {
                  doInHibernate(
                      _session -> {
                        ...
                        _session
                            .createQuery(
                                "update PostComment " + "set review = :review " + "where id = :id")
                            .setParameter("review", "Great")
                            .setParameter("id", 1L)
                            .executeUpdate();

                        LOGGER.info(
                            "Update {} prevented by explicit lock",
                            prevented.get() ? "was" : "was not");
                        bobLatch.countDown();
                      });
                } catch (Exception e) {
