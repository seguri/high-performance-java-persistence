...
public abstract class AbstractBatchPreparedStatementTest extends DatabaseProviderIntegrationTest {
  ...
  protected void batchInsert(Connection connection) throws SQLException {
    ...
    try (PreparedStatement postStatement =
            connection.prepareStatement("insert into post (title, version, id) values (?, ?, ?)");
      ...
      LOGGER.info(
          "{}.testInsert for {} using batch size {} took {} millis",
  ...
  protected void batchUpdate(Connection connection) throws SQLException {
    ...
    try (PreparedStatement postStatement =
            connection.prepareStatement("update Post set version = ? where id = ?");
        PreparedStatement postCommentStatement =
            connection.prepareStatement("update post_comment set version = ? where id = ?");
      ...
      LOGGER.info(
          "{}.testBulkUpdate for {} took {} millis",
          getClass().getSimpleName(),
          dataSourceProvider().getClass().getSimpleName(),
          TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startNanos));
    }
  ...
  protected void batchDelete(Connection connection) throws SQLException {
    ...
    try (PreparedStatement postStatement =
        ...
        PreparedStatement postCommentStatement =
            connection.prepareStatement("delete from post_comment where id = ?");
        Statement bulkUpdateStatement = connection.createStatement(); ) {
      int postCount = getPostCount();
      int postCommentCount = getPostCommentCount();
      ...
      LOGGER.info(
          "{}.testBulkDelete for {} took {} millis",
          getClass().getSimpleName(),
          dataSourceProvider().getClass().getSimpleName(),
          TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startNanos));
    }
