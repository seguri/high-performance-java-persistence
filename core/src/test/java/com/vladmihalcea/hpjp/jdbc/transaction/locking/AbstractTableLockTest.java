...
public abstract class AbstractTableLockTest extends AbstractTest {
  ...
  public static final String INSERT_DEPARTMENT =
      "insert into department (name, budget, id) values (?, ?, ?)";

  public static final String INSERT_EMPLOYEE =
      "insert into employee (department_id, name, salary, id) values (?, ?, ?, ?)";
  ...
  public void testPhantomReadAggregateWithTableLock() {
                  ...
                  () -> {
                    ...
                    doInJDBC(
                        daveConnection -> {
                          ...
                          try (PreparedStatement employeeStatement =
                              daveConnection.prepareStatement(insertEmployeeSql()); ) {
                            int employeeId = 7;
                            int index = 0;
    ...
    LOGGER.info(
        "Carol insert was {} prevented by lock", carolPreventedByLocking.get() ? "" : "not");
  ...
  @Table(
      name = "employee",
      indexes = @Index(name = "IDX_DEPARTMENT_ID", columnList = "department_id"))
